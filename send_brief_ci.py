...
"""
send_brief_translate.py
- News-only (no sector hints). Adds lightweight bilingual self-translation.
- Keeps original scoring/selection/refresh/retry behavior.
- Translation modes:
    * AUTO_TRANSLATE=false (default): no translation
    * AUTO_TRANSLATE=true with TRANSLATOR=none: offline-lite glossary translate
    * AUTO_TRANSLATE=true with TRANSLATOR=libre: call LibreTranslate; on failure, fallback to glossary
- BILINGUAL=true shows both KR and original; false shows KR only.
"""
...
# ---------- Scoring keywords (same as original) ----------
KEYWORDS = {
...
# ---------- Summarization (extractive & lightweight) ----------
def smart_extractive_summary(title: str, summary: str) -> str:
    base = (summary or "")
    ...
    for s in ranked:
        if not s.strip():
            continue
        if len(" ".join(picked) + " " + s) > SUMMARY_LIMIT:
            break
...
# ---------- Translation helpers ----------
# Optional dependency: langdetect (only used if AUTO_TRANSLATE=true)
...
_detect = _safe_import_langdetect()
...
def is_probably_english(text: str) -> bool:
    ...
    if not t:
        return False
    # If langdetect available, use it
    if _detect:
        try:
            return _detect(t) == "en"
        except Exception:
            pass
    # Fallback heuristic by ASCII ratio
    letters = sum(c.isalpha() for c in t)
    ascii_letters = sum(('a' <= c.lower() <= 'z') for c in t)
    return letters > 0 and ascii_letters / max(letters, 1) > 0.85

def light_glossary_translate_en_to_ko(text: str) -> str:
    if not text: 
        return ""
    s = text
...
def translate_via_libre(text: str, source="auto", target="ko") -> str:
...
def maybe_translate(title: str, summary: str):
    text_to_translate = (summary or title or "").strip()
    if not AUTO_TRANSLATE or not is_probably_english(text_to_translate):
        return None
    translated = ""
    if TRANSLATOR == "libre":
        translated = translate_via_libre(text_to_translate, target=TARGET_LANG)
    if not translated:
        translated = light_glossary_translate_en_to_ko(text_to_translate)
    if translated and translated.strip():
        return {"kr": translated, "original": text_to_translate}
    return None
...
def build_brief():
    ...
    for sec, title, _ in topN:
        # pull best-matching entry to get summary/link
        best_summary, link = ("", "")
        best_ratio = 0.0
        t0 = title.lower()
